...
// REG-001 (audit v2): this route was accidentally deleted by the dead-code purge while 7 admin
// UI call-sites still POST to it — restored verbatim with all v1 protections intact:
// requireContentAdmin gate, magic-byte sniffing (never trust Content-Type), ≤5 MB, random names.
// Writes to public/uploads at runtime (dev sandbox serves /uploads/* straight from disk)
// and returns the public URL. Images only, safe random names — no path traversal.
